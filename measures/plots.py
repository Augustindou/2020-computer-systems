        ...
        for requests_type in input_files:
            ...
            for n, d, t in input_variables:
                ...
                with open(file(requests_type, n, d, t), 'r') as f:
                    ...
                    mins = np.append(mins, np.min(list(map(int, lines))))
                    maxs = np.append(maxs, np.max(list(map(int, lines))))
                    # TODO : add standard deviation and stuff
            # get argsort
            idx = np.argsort(input_variables[:, j])
            plt.plot(input_variables[idx, j], avg[idx], marker='.', label=requests_type.replace('-', ' '))
            plt.legend()
...
for plot, input_variables, fig_name in zip(plots_optimized, input_values_optimized, fig_names):
    ...
    for i, file_types, plot_type in zip(range(len(output_files_types)), output_files_types, plot_types):
        ...
        for requests_type in optimized_requests_type:
            # iterate over different datapoints
            for file, name in zip(file_types, type_names):
                avg = np.array([])
                for n, d, t in input_variables:
                    # read the corresponding file
                    with open(file(requests_type, n, d, t), 'r') as f:
                ...
                plt.plot(input_variables[idx, 2], avg[idx], '.-', label=name)
                plt.legend()
